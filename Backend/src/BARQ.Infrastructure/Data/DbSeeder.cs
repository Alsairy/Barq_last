...
public static class DbSeeder
{
    ...
    private static async System.Threading.Tasks.Task SeedBillingPlansAsync(BarqDbContext context)
    {
        if (!await context.BillingPlans.AnyAsync())
        {
            var plans = new[]
            {
                new BillingPlan { Name = "Free", Price = 0, MaxUsers = 5, MaxProjects = 10, MaxStorageBytes = 1073741824, IsActive = true },
                new BillingPlan { Name = "Professional", Price = 29.99m, MaxUsers = 25, MaxProjects = 100, MaxStorageBytes = 10737418240, IsActive = true },
                new BillingPlan { Name = "Enterprise", Price = 99.99m, MaxUsers = 0, MaxProjects = 0, MaxStorageBytes = 0, IsActive = true }
            };
    ...
    private static async System.Threading.Tasks.Task SeedLanguagesAsync(BarqDbContext context)
    {
        if (!await context.Languages.AnyAsync())
        {
            var languages = new[]
            {
                new Language { Code = "en", Name = "English", NativeName = "English", Direction = "ltr", IsEnabled = true },
                new Language { Code = "ar", Name = "Arabic", NativeName = "العربية", Direction = "rtl", IsEnabled = true },
                new Language { Code = "es", Name = "Spanish", NativeName = "Español", Direction = "ltr", IsEnabled = true }
            };
